...
This document describes the continuous integration and deployment
workflows configured for the spatialcovariates package.

## Overview

The package uses GitHub Actions for automated testing, code quality
checks, and documentation deployment. All workflows are defined in
`.github/workflows/`.
...
The workflow tests on: - **Ubuntu Latest** (R release) - **Ubuntu
Latest** (R devel) - **macOS Latest** (R release) - **Windows Latest**
(R release)
...
**Linux**: - GDAL (geospatial data abstraction library) - GEOS (geometry
engine) - PROJ (coordinate transformation) - UDUNITS2 (unit
conversion) - SSL, CURL, XML2

**macOS**: - GDAL, PROJ, GEOS, UDUNITS via Homebrew

**Windows**: - Rtools (automatically configured)
...
If the check fails: 1. Review the uploaded check results artifact 2.
Examine `testthat.Rout` for test failures 3. Fix issues locally and push
again

### 2. test-coverage.yaml

**Trigger**: Push to main/master, pull requests **Purpose**: Measure and
report test coverage **Status Badge**:
...
Coverage settings in `codecov.yml`: - Target: 80% overall coverage -
Project threshold: 1% - Patch threshold: 1% - Ignores: Package
...
**Trigger**: Push to main/master, releases, manual dispatch **Purpose**:
Build and deploy documentation website **Website**:
...
Site configuration in `_pkgdown.yml`: - Bootstrap 5 theme - Cosmo
bootswatch style - Organized function reference - Custom navigation
...
**Trigger**: Push to main/master, pull requests **Purpose**: Enforce
...
Linting rules in `.lintr`: - Max line length: 120 characters - Allows
flexible object naming - Skips complexity checks - Ignores package
documentation
...
Automatically provided by GitHub Actions. Used for: - Installing
packages from GitHub - Deploying to GitHub Pages - Uploading artifacts
...
**Ubuntu**: Update `apt-get install` commands in workflow **macOS**:
Update `brew install` commands **Windows**: Usually handled by Rtools
automatically
...
Options: - Only failures - All workflows - Per-workflow basis
...
Open an issue if: - Workflows fail unexpectedly - New platform support
needed - Additional checks required - Performance issues
